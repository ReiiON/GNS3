...
def connect_device(device):
    try:
        # Obtener las credenciales de los secretos o de un archivo de configuración
        username = os.getenv('ROUTER_USER')  # Usar variable de entorno para el usuario
        password = os.getenv('ROUTER_PASSWORD')  # Usar variable de entorno para la contraseña

        net_connect = netmiko.ConnectHandler(
            device_type=device['device_type'],
            host=device['host'],
            username=username,  # Nombre de usuario desde los secretos
            password=password,  # Contraseña desde los secretos
...
def main():
    ...
    for device in devices:
        ...
        if net_connect:
            router_name = device.get('router_name', device['host'])  # Obtener 'router_name' o usar el 'host' si no existe
